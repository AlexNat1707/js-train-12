...
function getKeysStartingWith(dictionary, letter) {
  ...
  for (const [key, value] of dictionary) {
    if (typeof key === "string" && key.startsWith(letter)) {
      arr.push(key)
    }
  }
  return arr;
...
function sortByValues(dictionary) {
  const entriesArray = Array.from(dictionary.entries());
...
function convertDictionaryToSet(dictionary) {
  ...
  for (const key of dictionary) {
    if (typeof dictionary[key] === "string") {
      set.add(dictionary[key].toUpperCase());
    } else if (typeof dictionary[key] === "number") {
      set.add(dictionary[key] + 1);
    }
  }
  return set;
...
function convertSetToDictionary(set) {
  ...
  for (const value of set) {
    ...
    dictionary.set(key, firstCharCode);
  }
  return dictionary;
